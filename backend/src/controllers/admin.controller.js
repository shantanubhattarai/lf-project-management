...
router.delete("/projects/update", function (req, res, next) {
  let sqlQuery = `update projects set is_deleted='t' where id=${req.body.project_id}`;
...
router.put("/projects/update", function (req, res, next) {
  let sqlQuery = `update projects SET name=${req.body.name}, description=${req.body.description}, project_manager=${req.body.project_manager_id} where id = ${req.body.project_id}`;
